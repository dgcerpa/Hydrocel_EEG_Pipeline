...
%   msg     - descriptive message of outcome
    ...
    try
        ...
        pre1AccCol = find(strcmp(headers, 'preg1.ACC'));
        pre2AccCol = find(strcmp(headers, 'preg2.ACC'));
        ...
        % Check that valid event codes remain after cleaning
        valid_codes = {'11','12','13','21','22','23'};
        n_valid = sum(ismember({EEG.event.type}, valid_codes));
        fprintf(fid_subj, 'Valid event codes remaining: %d\n', n_valid);
        
        if n_valid == 0
            msg = 'No valid event codes (11-23) remain after cleaning - skipping';
            fprintf(fid_subj, 'SKIPPED: %s\n', msg);
            movefile(fullfile(cleaned_dir, cleaned_filename), problematic_dir);
            fclose(fid_subj);
            return;
        end
        ...
        % Create derived bins (combinations of congruencia × valencia)
        fprintf(fid_subj, '\nCreating derived bins (6-11)...\n');
        ERP = pop_binoperator(ERP, { ...
            'b6  = (b1+b3)/2 label Congruente Positiva', ...
            'b7  = (b1+b4)/2 label Congruente Neutra', ...
            'b8  = (b1+b5)/2 label Congruente Negativa', ...
            'b9  = (b2+b3)/2 label Incongruente Positiva', ...
            'b10 = (b2+b4)/2 label Incongruente Neutra', ...
            'b11 = (b2+b5)/2 label Incongruente Negativa' ...
            });
        fprintf(fid_subj, 'Derived bins created: b6-b11\n');
        
        % Create averaged channel RPOI
        fprintf(fid_subj, '\nCreating averaged channel RPOI...\n');
        ERP = pop_erpchanoperator(ERP, ...
            {'ch65 = (ch31 + ch32 + ch33 + ch34 + ch35 + ch36 + ch37 + ch38 + ch39 + ch40 + ch43)/11 label RPOI'}, ...
            'ErrorMsg', 'popup', ...
            'KeepLocations', 1, ...
            'Warning', 'on');
        fprintf(fid_subj, 'Channel RPOI created\n');
